...
// type Problem = {
//   id: string;
//   circles: number; // number of circles
//   partsPerCircle: number; // denominator
//   shadedParts: number; // numerator
//   simplified: string; // expected minimized result (e.g. "1 1/2", "2", "2 1/4")
// };
...
//   { id: "p1", circles: 2, partsPerCircle: 4, shadedParts: 6, simplified: "1 1/2" },
//   { id: "p2", circles: 2, partsPerCircle: 3, shadedParts: 5, simplified: "1 2/3" },
//   { id: "p3", circles: 2, partsPerCircle: 5, shadedParts: 10, simplified: "2" },
//   { id: "p4", circles: 3, partsPerCircle: 4, shadedParts: 9, simplified: "2 1/4" },
// ];

// const DEFAULT_HINT =
//   "Count all shaded parts (numerator) and total parts in one circle (denominator). Then reduce the fraction.";

// /* ---------------- Circle Tiny Component ---------------- */
// const Circle: React.FC<{ parts: number; shaded: number }> = ({ parts, shaded }) => {
...
//         const largeArc = (2 * Math.PI) / parts > Math.PI ? 1 : 0;
...
//   // const DATA = useMemo(() => (data?.length ? data : DEFAULT_DATA), [data]);
//   const DATA = DEFAULT_DATA;
//   const help = hint ?? DEFAULT_HINT;

//   const [improper, setImproper] = useState<string[]>(() => DATA.map(() => ""));
//   const [simplified, setSimplified] = useState<string[]>(() => DATA.map(() => ""));
...
//       const userSimplified = simplified[i].trim();

//       return (
//         userImproper === expectedImproper &&
//         userSimplified.toLowerCase() === p.simplified.toLowerCase()
//       );
//     });
//     setOk(results);
...
//         <p className="text-sm text-slate-600">Which fraction? Remove the wholes.</p>
//       </div>
...
//               <input
//                 className="border-b-2 border-dashed focus:outline-none text-center w-16"
//                 value={improper[pi]}
//                 onChange={(e) =>
//                   setImproper((prev) => {
//                     const cp = [...prev];
//                     cp[pi] = e.target.value;
//                     return cp;
//                   })
//                 }
//               />
...
//             <div className="flex items-center gap-2">
//               <input
//                 className="border-b-2 border-dashed focus:outline-none text-center w-20"
//                 value={simplified[pi]}
//                 onChange={(e) =>
//                   setSimplified((prev) => {
//                     const cp = [...prev];
...
// const InlineFractionInput: React.FC<{
//   value: string; // e.g. "1 1/2" or "6/4"
...
//         }
//         className={`w-8 text-center border-b ${border}`}
//       />
...
//           }
//           className={`w-10 text-center border-b ${border}`}
//         />
//         <input
//           type="text"
//           value={den}
...
//       return (
//         userImproper === expectedImproper &&
//         userSimplified.toLowerCase() === p.simplified.replace(/\s/g, "").toLowerCase()
//       );
//     });
//     setOk(results);
//     setStatus(results.every(Boolean) ? "match" : "wrong");
...
//                 );
//                 return (
//                   <Circle key={ci} parts={p.partsPerCircle} shaded={shadedInThis} />
//                 );
//               })}
//             </div>

//             {/* Inline fraction: improper = simplified */}
//             <div className="flex items-center gap-2">
//               <InlineFractionInput
//                 value={improper[pi]}
//                 onChange={(val) =>
          ...
          <div key={p.id} className="flex flex-col items-center gap-3">
            ...
            <div className="flex gap-2">
              {Array.from({ length: p.circles }).map((_, ci) => {
                ...
                return <Circle key={ci} parts={p.partsPerCircle} shaded={shadedInThis} />;
              })}
            </div>
