  ...
  const summary =
    status
      ? {
          ...
          bgColor: status === "match" ? "bg-green-100" : "bg-red-100",
          borderColor: status === "match" ? "border-green-600" : "border-red-600",
        }
      : null;

  return (
    <div className="flex flex-col space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 pr-20 pl-20 pt-10 pb-10 gap-4">
        {problemsJSON.map((p, idx) => (
          <div key={p.id} className="p-4 rounded-lg shadow-sm bg-[#FFF7ED]  border border-gray-200">
