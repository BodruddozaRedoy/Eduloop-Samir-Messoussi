...
const DEFAULT_DATA: Required<Card>[] = [
  { id: "c1", prompt: "10 to 8 in the morning", options: ["07:50", "19:50"], correct: 0 },
  { id: "c2", prompt: "5 past 8 in the evening", options: ["11:05", "23:05"], correct: 1 },
  { id: "c3", prompt: "10 to 4 at night", options: ["04:10", "16:10"], correct: 0 },
  { id: "c4", prompt: "5 to 5 in the afternoon", options: ["04:55", "16:55"], correct: 1 },
  { id: "c5", prompt: "5 past 7 in the evening", options: ["07:05", "19:05"], correct: 1 },
];
...
const ArrType: React.FC<Props> = ({ data, hint }) => {
  ...
  const CARDS = useMemo<Required<Card>[]>(() => {
    ...
    return src.map((c, i) => {
      ...
      const safeCorrect =
        typeof c?.correct === "number" && c.correct >= 0 && c.correct < options.length ? c.correct : 0;
  ...
  const [picked, setPicked] = useState<(number | null)[]>(() => CARDS.map(() => null));
  ...
  const tickTone = (cardIdx: number, optIdx: number) => {
    ...
    if (!wasChecked) return isPicked ? "border-orange-500" : "border-orange-300";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-5">
        {CARDS.map((card, ci) => (
          <div key={card.id} className="rounded-md bg-orange-50 p-4 shadow-[0_1px_0_rgba(0,0,0,0.05)]">
            <div className="mb-3 text-[13px] font-semibold text-slate-700">{card.prompt}</div>

            <div className="space-y-3">
              {(card.options ?? []).map((time, oi) => (
                <div key={`${card.id}-${time}-${oi}`} className="flex flex-col items-center gap-2">
                  {/* Time chip */}
                  <div
                    className={`w-20 rounded-md border px-2 py-1 text-center text-sm tabular-nums ${boxTone(ci, oi)}`}
                  >
                    {time}
                  </div>
                  ...
                  >
                    <span
                      className={`h-3 w-3 rounded-[2px] ${picked[ci] === oi ? "bg-orange-500" : "bg-transparent"}`}
                    />
                  </button>
                </div>
              ))}
              ...
              {(!card.options || card.options.length === 0) && (
                <div className="text-xs text-slate-500">No options provided.</div>
              )}
            </div>
          </div>
        ))}
